...
//    }
    ...
    [self sendMessage:message
            recipient:failedRecipient
               thread:errorMessage.thread
             attempts:OWSMessageSenderRetryAttempts
              success:^{
                  DDLogDebug(@"Successfully resent message after error to: %@", failedRecipient.uniqueId);
              }
              failure:^(NSError *error) {
                  DDLogDebug(@"Failed to resend message after error to: %@", failedRecipient.uniqueId);
              }];
...
#warning XXX Bounce off CCSM for info?
...
//            [self saveMessage:message withState:TSOutgoingMessageStateAttemptingOut];
        ...
        // Message send.
            ...
            // catch message to self
            if (thread.participants.count == 1 && [[thread.participants lastObject] isEqualToString:TSAccountManager.sharedInstance.myself.uniqueId]) {
                [self handleSendToMyself:message];
                return;
            }
            
            // Not to self, proceed with the send
            NSError *error;
            NSArray<SignalRecipient *> *recipients = [self getRecipients:thread.participants error:&error];
            ...
            for (SignalRecipient *rec in recipients) {
                ...
                if (![[rec uniqueId] isEqualToString:TSAccountManager.sharedInstance.myself.uniqueId]) {
...
//              preKeyBundle:exception.userInfo[TSInvalidPreKeyBundleKey]] save];
            [[TSInvalidIdentityKeySendingErrorMessage
              ...
              inThread:thread
              forRecipient:exception.userInfo[TSInvalidRecipientKey]
              preKeyBundle:exception.userInfo[TSInvalidPreKeyBundleKey]] acceptNewIdentityKey];
            DDLogDebug(@"Automatically accepted key change.");
...
- (void)handleSendToMyself:(TSOutgoingMessage *)outgoingMessage
{
    ...
    if (!(outgoingMessage.plainTextBody || outgoingMessage.hasAttachments)) {
        DDLogDebug(
    ...
    [self.dbConnection readWriteWithBlock:^(YapDatabaseReadWriteTransaction *transaction) {
        TSThread *thread = [TSThread getOrCreateThreadWithID:TSAccountManager.sharedInstance.myself.uniqueId transaction:transaction];
        [thread saveWithTransaction:transaction];
        TSIncomingMessage *incomingMessage =
        [[TSIncomingMessage alloc] initWithTimestamp:(outgoingMessage.timestamp + 1)
                                            inThread:thread
                                            authorId:TSAccountManager.sharedInstance.myself.uniqueId
                                         messageBody:outgoingMessage.body
                                       attachmentIds:outgoingMessage.attachmentIds
                                    expiresInSeconds:outgoingMessage.expiresInSeconds];
        incomingMessage.plainTextBody = outgoingMessage.plainTextBody;
