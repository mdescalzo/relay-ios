...
{
    NSString *tssURL = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"TSS_Server_URL"];
    NSURLSessionConfiguration *sessionConf = NSURLSessionConfiguration.ephemeralSessionConfiguration;
    NSURL *baseURL = [[NSURL alloc] initWithString:tssURL];
            ...
            failure:(void (^)(NSURLSessionDataTask *task, NSError *error))failureBlock
{
    ...
    if ([request isKindOfClass:[TSVerifyCodeRequest class]]) {
        ...
        [self.operationManager PUT:[tssURL stringByAppendingString:request.URL.absoluteString]
        ...
        if ([request.HTTPMethod isEqualToString:@"GET"]) {
            [self.operationManager GET:[tssURL stringByAppendingString:request.URL.absoluteString]
                            parameters:request.parameters
                              progress:nil
                               success:success
                               failure:failure];
        } else if ([request.HTTPMethod isEqualToString:@"POST"]) {
            [self.operationManager POST:[tssURL stringByAppendingString:request.URL.absoluteString]
                             parameters:request.parameters
                               progress:nil
                                success:success
                                failure:failure];
            ...
            [self.operationManager DELETE:[tssURL stringByAppendingString:request.URL.absoluteString]
                               parameters:request.parameters
                                  success:success
                                  failure:failure];
        } else {
