...
private class RelayCallData: NSObject {
    ...
    deinit {
        Logger.debug("[RelayCallData] deinit")
    }
...
@objc public class CallService: NSObject, CallObserver, PeerConnectionClientDelegate {
    ...
    private let accountManager = TSAccountManager.sharedInstance()
    private let messageSender = Environment.shared().messageSender
    private let contactsManager = Environment.shared().contactsManager
...
//            }
    ...
    @objc public required override init() {
        // TODO: Investigate NotificationsAdapter
...
//        SwiftSingletons.register(self)
    ...
    func handleOutgoingCall(_ call: RelayCall) -> Promise<Void> {
        ...
        guard self.call == nil else {
            ...
            // TODO: Investifate OWSProdError functionality
...
//                OWSProdError(OWSAnalyticsEvents.callServicePeerConnectionAlreadySet(), file: #file, function: #function, line: #line)
            ...
            // TODO: Investigate/implement TURN-only functionality
//            let useTurnOnly = Environment.preferences().doCallsHideIPAddress()
    ...
    public func handleMissedCall(_ call: RelayCall) {
        ...
        if let callRecord = call.callRecord {
            ...
            call.callRecord = TSCall(timestamp: NSDate.ows_millisecondTimeStamp(),
                                     withCallId: call.callUUID.uuidString,
...
//                                     in: call.thread)
     ...
     */
    public func handleRemoteBusy(thread: TSThread, callId: String) {
        ...
        guard call.callUUID.uuidString == callId else {
            Logger.warn("\(self.logTag) ignoring mismatched call: \(callId) currentCall: \(call.callUUID.uuidString) in \(#function)")
            return
        }

        // TODO: Validate necessity in our environment
...
//        }
    ...
    public func handleReceivedOffer(thread: TSThread, callId: UInt64, sessionDescription callerSessionDescription: String) {
        ...
        // TODO: Investigate OWSIdentityManager - untrustedIdenity usage
...
//            OWSProdError(OWSAnalyticsEvents.callServiceCallMissing(), file: #file, function: #function, line: #line)
    ...
    /**
     * User chose to answer call referred to by call `callUUID`. Used by the Callee only.
     *
     * Used by notification actions which can't serialize a call object.
     */
    @objc public func handleAnswerCall(callUUID: UUID) {
        SwiftAssertIsOnMainThread(#function)
...
//            OWSProdError(OWSAnalyticsEvents.callServiceCallMissing(), file: #file, function: #function, line: #line)
        ...
        guard call.callUUID == callUUID else {
            // This should never happen; return to a known good state.
            owsFail("\(self.logTag) callLocalId:\(callUUID) doesn't match current calls: \(call.callUUID)")
//            OWSProdError(OWSAnalyticsEvents.callServiceCallIdMismatch(), file: #file, function: #function, line: #line)
            handleFailedCurrentCall(error: CallError.assertionError(description: "\(self.logTag) callLocalId:\(callUUID) doesn't match current calls: \(call.callUUID)"))
            return
        }
...
//            OWSProdError(OWSAnalyticsEvents.callServicePeerConnectionMissing(), file: #file, function: #function, line: #line)
     ...
     *
     * The call is referred to by call `callUUID`, which is included in Notification actions.
     *
     * Incoming call only.
     */
    public func handleDeclineCall(callUUID: UUID) {
        SwiftAssertIsOnMainThread(#function)
...
//            OWSProdError(OWSAnalyticsEvents.callServiceCallMismatch(), file: #file, function: #function, line: #line)
     ...
     */
    // TODO: Investigate necessity of dataChannel handling in our environment
