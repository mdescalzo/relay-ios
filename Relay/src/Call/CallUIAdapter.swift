...
extension CallUIAdaptee {
    internal func showCall(_ call: RelayCall) {
        ...
        guard #available(iOS 10.0, *) else {
            Logger.debug("\(#function) attempted to showCall from iOS < 10.")
            return
        }
...
@objc public class CallUIAdapter: NSObject, CallServiceObserver {
    ...
    internal let callService = Environment.shared().callService
    ...
    @objc public func answerCall(callUUID: UUID) {
        SwiftAssertIsOnMainThread(#function)
        adaptee.answerCall(callUUID: callUUID)
    }
    ...
    @objc public func declineCall(callUUID: UUID) {
        SwiftAssertIsOnMainThread(#function)
        adaptee.declineCall(callUUID: callUUID)
    }
